...
public class RewardServiceTest extends JPATestCase {
	public void testNominate() {
		...
		RewardItem nominateItem = RewardItemHelper.getDefaultNominateRewardItem(injector);
		// prepare default caller
		SysUser caller = UserHelper.getDefaultUser(injector);
	
		Date now = DateUtil.getTime();
		Reward reward = rewardLogic.awardFromRewardItem(caller,nominateItem.getId(), now);
		// check reward
		...
		//judgeLogic.cloneJudgesFromRewardItemToReward(caller, nominateItem.getId(), reward.getId());
		...
		for (int i = 0; i < list.size(); i++) {
			System.out.println(list.get(i).getStaff().getName()+"-------"+list.get(i).getStaff().getId());
		}
		...
		System.out.println("caller.getStaff()========"+caller.getStaff().getId());
