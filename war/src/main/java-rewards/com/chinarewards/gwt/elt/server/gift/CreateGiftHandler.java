...
public class CreateGiftHandler extends
	...
	public AddGiftResponse execute(AddGiftRequest action,
		...
		logger.debug("GiftAddResponse , parameter:"
				+ action.getGiftVo().toString());
		logger.debug("GiftAddResponse ,rewardId=" + action.getGiftVo().getId());
		GiftVo giftVo = action.getGiftVo();
		Gift gift = assembleParameter(giftVo);
		...
		uc.setUserRoles(UserRoleTool.adaptToRole(action.getUserSession()
				.getUserRoles()));
		Gift createdItem = giftService.save(uc, gift);
		return new AddGiftResponse(createdItem.getId());
	}

	// Convert from GiftVo to GeneratorGiftModel.
	private Gift assembleParameter(GiftVo giftVo) {
		// 奖项参数
		Gift parameter = new Gift();
		parameter.setId(giftVo.getId());
		parameter.setName(giftVo.getName());
