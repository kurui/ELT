...
public class FileExcelUtil {
	...
	public static void createXLSFile(ArrayList<Object> title,
		...
		try {
			...
			response.setHeader("Content-disposition", "attachment;filename="
					+ sheetName + ".xls");// 设定输出文件头

			response.setContentType("application/msexcel");// 设定输出类型

			doWriteXLS(title, lists, sheetName, os);
	...
	private static void doWriteXLS(ArrayList<Object> title,
			ArrayList<ArrayList<Object>> lists, String sheetName,
			OutputStream os) {
		try {
	...
	private static void ___doWriteXLS(ArrayList<Object> title,
			ArrayList<ArrayList<Object>> lists, String sheetName,
			OutputStream os) {
		try {
			WritableWorkbook wwb = Workbook.createWorkbook(os); // 创建Excel工作薄
