 ...
 * @since 1.5.2 
 */
public class ImportGiftCodeData {
...
//};
	 ...
	 * @throws Exception
	 */
	public static List<ParserCode> getAllImportGiftCode() throws Exception {
		List<ParserCode> result = new ArrayList<ParserCode>();
		Class<?> parser = Class.forName(ImportGiftCodeData.class.getName());
		Field[] fields = parser.getDeclaredFields();
		for(int j=0 ; j<fields.length ; j++){
            String name = fields[j].getName();
            Field field = parser.getField(name);
            Object[] value = (Object[])field.get(parser);
            ImportGiftCodeData data = new ImportGiftCodeData();
